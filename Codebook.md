...
*An overview of the relationship between the dataset Samsung files, along with helpful processing tips was provided by David Hood, Coursera: Getting and Cleaning Data Course, discussion thread: (https://class.coursera.org/getdata-007/forum/thread?thread_id=49)
...
1.	For the training and test data, information from 4 associated files will ultimately need to be assembled to create the second tidy dataset. For example, the following training-related files were needed to assemble and process the training data (the corresponding ‘test’ files will be needed for the test data):
...
2.	Load training dataset and associated files; load test dataset and associated files
3.	Merge the training and the test datasets using rbind(). That is, the measurement data from the test and training files will be merged; the subject data from the test and training files will be merged; and the activity codes from the test and training data will be merged.
4.    The variable names for each column in the merged measurement dataframe are located in the 'features.txt' file. Need to read this file in and set the colnames using the variable names in this file.
